...
[https://www.python.org/dev/peps/pep-0008/ Python style guidelines]

==Plugins specification==

First, you must understand that a qBittorrent search engine plugin is actually a Python class file whose task is to contact a search engine website (e.g. [http://www.mininova.org Mininova.org]), parse the results display by the web page and print them on stdout with the following syntax:
...
* ''[https://docs.python.org/2/library/htmlparser.html HTMLParser]/[https://docs.python.org/3/library/html.parser.html html.parser]'': Builtin python parser which replaces deprecated SGMLParser. Mostly similar to the SMGLParser '''[ADVISED METHOD]'''
...
==Examples==
Do not hesitate to use the official search engine plugins as an example. They are available [https://github.com/qbittorrent/qBittorrent/tree/master/src/searchengine/nova/engines here]. 
* ''kickasstorrents.py'' uses ''json'' module
* ''torrentreactor.py'' uses ''HTMLParser'' module

==Test your plugin==
It is better to test your plugin while developing it, before installing it in qBittorrent. Hence, we advise that you download [http://hydr0g3n.free.fr/search_engine.zip this archive]. Unzip it and you will get the following structure:
...
Once you managed to write a search engine plugin for qBittorrent that works, please post in on this wiki page [http://plugins.qbittorrent.org here] so that the other users can use it too if they want. If you are lucky, your plugin may also be included in a future qBittorrent release.
