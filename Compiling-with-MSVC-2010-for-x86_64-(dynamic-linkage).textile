...
<blockquote>
Note: It is possible to use 64-bit compiler collection included in WinSDK 7.1. But if you already have *ANY* edition of Visual Studio installed with SP1 applied you won't be able to install it. Thus, using WinSDK is out of this article's scope.
</blockquote>
<blockquote>
...
** 0.16.5 is used in this article.
...
<blockquote>
Last command will run a short test to test if Zlib is working.
</blockquote>
...
COPY /Y *.dll T:\install\zlib\bin\
COPY /Y zdll.lib T:\install\zlib\lib\
COPY /Y zconf.h T:\install\zlib\include\
COPY /Y zlib.h T:\install\zlib\include\
</code>
</pre>
...
<pre>
<code>
perl Configure VC-WIN64A threads shared zlib-dynamic -IT:\install\zlib\include -LT:\install\zlib\lib --prefix=T:\install\openssl
.\ms\do_win64a.bat
</code>
</pre>
# Edit @.\ms\ntdll.mak@ and do the following changes:
** Find line starting with @CFLAG=@ and on that line:
*** Replace @/Ox@ with @/O2 /favor:blend /GL /Y- /MP@
...
@SET "PATH=%PATH%;T:\install\Zlib\bin"@
<blockquote>
Technical note: without this OpenSSL won't be able to use Zlib during test phase.
</blockquote>
@nmake -f .\ms\ntdll.mak test@
...
CD .\tools\build\v2
.\bootstrap.bat vc10
.\b2.exe --toolset=msvc architecture=x86 address-model=64 --prefix=T:\install\bjam link=shared runtime-link=shared variant=release debug-symbols=off warnings=off warnings-as-errors=off inlining=full optimization=speed "cflags=/O2 /GL /Gy /MP /favor:blend" "linkflags=/NOLOGO /OPT:REF /OPT:ICF=5 /LTCG" install
CD ..\..\..\
</code>
</pre>
...
<pre>
<code>
SET "PATH=%PATH%;T:\install\bjam\bin"
bjam -j2 -q --with-system --with-date_time --toolset=msvc --layout=system --prefix=T:\install\boost link=shared runtime-link=shared variant=release debug-symbols=off threading=multi address-model=64 host-os=windows target-os=windows embed-manifest=on architecture=x86 warnings=off warnings-as-errors=off inlining=full optimization=speed "cflags=/O2 /GL /MP /favor:blend" "linkflags=/NOLOGO /OPT:REF /OPT:ICF=5 /LTCG" install
COPY /Y *.jam T:\install\boost
...
<code>
SET "PATH=%PATH%;T:\install\Zlib\bin"
.\configure.exe -release -shared -opensource -confirm-license -platform win32-msvc2010 -arch windows -ltcg -no-fast -exceptions -no-accessibility -stl -no-xmlpatterns -no-sql-mysql -no-sql-psql -no-sql-oci -no-sql-odbc -no-sql-tds -no-sql-db2 -no-sql-sqlite -no-sql-sqlite2 -no-sql-ibase -no-qt3support -no-opengl -no-openvg -graphicssystem raster -system-zlib -qt-libpng -no-libmng -no-libtiff -qt-libjpeg -no-gif -no-dsp -no-vcproj -no-incredibuild-xge -plugin-manifests -process -mp -no-rtti -no-3dnow -mmx -sse -sse2 -openssl -no-dbus -no-phonon -no-phonon-backend -no-multimedia -no-audio-backend -no-webkit -no-script -no-scripttools -no-declarative -no-declarative-debug -no-style-s60 -no-style-windowsmobile -no-style-windowsce -no-style-cde -no-style-motif -qt-style-cleanlooks -qt-style-plastique -qt-style-windows -qt-style-windowsxp -qt-style-windowsvista -no-native-gestures -no-directwrite -qmake -nomake examples -nomake demos -nomake tools -nomake translations -nomake docs -I T:\install\OpenSSL\include -L T:\install\OpenSSL\lib -I T:\install\Zlib\include -L T:\install\Zlib\lib -prefix T:\install\Qt
nmake sub-src
</code>
</pre>
# Install Qt:
...
# Unpack qbittorrentinto @\sources\qbt\@.
# Open @Visual Studio x64 Win64 Command Prompt (2010)@.
...
MD build
CD build
SET "PATH=%PATH%;T:\install\Qt\bin;T:\install\Zlib\bin"
qmake -r ../qbittorrent.pro "CONFIG+=warn_off rtti ltcg mmx sse sse2" "CONFIG-=3dnow"
...
<code>
CD .\src\release
IF EXIST T:\install\qBittorrent RD /S /Q T:\install\qBittorrent
MD T:\install\qBittorrent
COPY /Y *.exe T:\install\qBittorrent\
...
FOR %X IN (codecs iconengines imageformats) DO (
    MD T:\install\qBittorrent\%X
    COPY /Y T:\install\Qt\plugins\%X\*.dll T:\install\qBittorrent\%X\
)
COPY /Y T:\install\OpenSSL\bin\*.dll T:\install\qBittorrent\
COPY /Y T:\install\libtorrent\lib\*.dll T:\install\qBittorrent\
COPY /Y T:\install\Boost\lib\*.dll T:\install\qBittorrent\
COPY /Y T:\install\Zlib\bin\*.dll T:\install\qBittorrent\
</code>
</pre>
