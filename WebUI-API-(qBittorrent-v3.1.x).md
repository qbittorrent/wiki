...
Requires known torrent hash, get 'em from [torrent list](#get-torrent-list).
...
`autorun_program` - program path/name/arguments to run if `autorun_enabled` is enabled; path is separated by slashes; you can use `%f` and `%n` arguments, which will be expanded by qBittorrent as path_to_torrent_file and torrent_name (from the GUI; not the .torrent file name) respectively \
...
`enable_utp` - true if uTP protocol should be enabled; this option is only available in qBittorrent built against libtorrent version 0.16.X and higher \
`limit_utp_rate` - true if `[du]l_limit` should be applied to uTP connections; this option is only available in qBittorrent built against libtorrent version 0.16.X and higher \
...
`anonymous_mode` - if true anonymous mode will be enabled; read more [here](Anonymous-Mode); this option is only available in qBittorrent built against libtorrent version 0.16.X and higher \
...
`proxy_peer_connections` - true if peer and web seed connections should be proxified; this option will have any effect only in qBittorrent built against libtorrent version 0.16.X and higher \
...
1. JSON parsing bugs
   * When [setting qBittorrent preferences](#set-qbittorrent-preferences) JSON values, containing `:` semicolons will be disregarded; this mostly affects Windows users, whose paths start with `DiskName:\`
   * When [setting qBittorrent preferences](#set-qbittorrent-preferences) JSON bool lists (e.g. `"download_in_scan_dirs":[false,true]`) will be treated as all bool values in the list are `false`, this doesn't affect bool values outside JSON lists
